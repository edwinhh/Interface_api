...
# myp=[]
# myr=[]
url='http://gis-int.intsit.sfdc.com.cn:1080/tip/api'
#url='http://gis-int.intsit.sfdc.com.cn:1080/tipcor/api'
name = os.path.basename(__file__).split('.')[0]

test=1
adcode="e:/项目/地理编码/数据/adcode.csv"
dir="e:/项目/输入提示/c++/temp/"
filelist=[]
...
class geo_Mutest(TestAbstract):
    ...
    def opennull(self,file):
        now = time.strftime('%Y-%m-%d-%H_%M_%S', time.localtime(time.time()))
        file3 = os.path.dirname(os.path.dirname(__file__)) + '/report' + '/' + name + "_" + now+"_Null_"+file
        #os.mknod(rfile)
        #with open(file2, 'w+', encoding='utf_8')as f
        opennull = open(file3, 'a', encoding='utf_8')
        return opennull
    ...
    def report(self,f,err, url, data, res):
            ...
            for i in range(len(res)):
                ...
                if "adname" in res[i]:
                    ...
                    if "null" in res["result"]["POISet"][0]["adname"]:
                        #print (res1["result"]["POISet"][0]["adname"])
